...
# Render Free tier wipes /opt/render/project/src on every deploy.
# We keep ALL mutable data on the mounted disk at /data (render.yaml: disk.mountPath).
# On first boot after migration, we move any existing DB/files to /data automatically.

def _migrate_to_persistent_disk():
    """
    If old data exists in the project directory (pre-disk-migration),
    move it to /data so no orders or receipts are lost.
    """
    data_root = Path("/data")
    if not data_root.exists():
        # Running locally or disk not mounted — skip silently
        return

    new_db   = data_root / "mathkarati_payments.db"
    new_stor = data_root / "storage"

    # --- DB migration ---
    old_db_candidates = [
        Path(_BASE) / "mathkarati_payments.db",
        Path("/opt/render/project/src/mathkarati_payments.db"),
    ]
        ...
        for old_db in old_db_candidates:
            if old_db.exists() and old_db != new_db:
                ...
                logging.getLogger(__name__).warning(
                    f"[MIGRATION] Copied DB {old_db} → {new_db}"
                )
                break
    ...
    old_stor_candidates = [
        Path(_BASE) / "storage",
        Path("/opt/render/project/src/storage"),
    ]
    if not new_stor.exists():
        for old_stor in old_stor_candidates:
            if old_stor.exists() and old_stor != new_stor:
                import shutil
                shutil.copytree(str(old_stor), str(new_stor))
                logging.getLogger(__name__).warning(
                    f"[MIGRATION] Copied storage {old_stor} → {new_stor}"
                )
                break

_migrate_to_persistent_disk()
...
def health():
    ...
    disk_ok  = Path("/data").exists() and Path("/data").is_mount() or Path("/data").exists()
    db_path  = os.environ.get("DB_PATH", "")
    stor_dir = os.environ.get("STORAGE_DIR", "")
    return jsonify({
        "status": "ok",
        "version": "29.0",
        "font": pipeline._font,
...
def create_order_route():
    ...
    if not email and not phone:
        return jsonify({"error": "يجب تقديم البريد الإلكتروني أو رقم الهاتف"}), 400
...
def admin_approve(order_id):
    ...
    if not pptx_src.exists():
        # ملف العرض المؤقت ضاع (تم نشر تحديث منذ التوليد).
        # نرجع رسالة واضحة للمدير بدل 500 غامض.
        return jsonify({
            "error": "ملف العرض الأصلي غير موجود — الطالب يجب أن يُعيد التوليد",
            "hint": "اطلب من الطالب زيارة الموقع وتوليد العرض مجدداً، ثم أعد رفع الوصل",
            "presentation_id": pid,
        }), 409
